...
void ekf_update(State &s, const Vector3d &z, const Matrix3d &R) {
 ...
 return_type load_data(const json &in, std::string topic = "") override {
    ...
    if (topic == "imu") { 
      ...
      // Process IMU Data if both gyro and accel are ready
        ...
        if(!_first_imu_frame){
          ...
          // Vettore Offset (lever arm)H_initial_walker_aruco
  ...
  void set_params(const json &params) override {
    ...
    if (params.contains("wheel_radius_left")) _conf.wheel_radius_left = params["wheel_radius_left"];
    if (params.contains("wheel_radius_right")) _conf.wheel_radius_right = params["wheel_radius_right"];
    if (params.contains("baseline")) _conf.baseline = params["baseline"];
    if (params.contains("ticks_per_rev")) _conf.ticks_per_rev = params["ticks_per_rev"];

    if(params.contains("sigma_v")) _conf.sigma_v = params["sigma_v"];
    if(params.contains("sigma_w")) _conf.sigma_w = params["sigma_w"];
    if(params.contains("sigma_acc")) _conf.sigma_acc = params["sigma_acc"];
    if(params.contains("sigma_enc_rot")) _conf.sigma_enc_rot = params["sigma_enc_rot"];
    if(params.contains("sigma_gyro")) _conf.sigma_gyro = params["sigma_gyro"];
    if(params.contains("sigma_rs_pos")) _conf.sigma_rs_pos = params["sigma_rs_pos"];
    if(params.contains("sigma_rs_ang")) _conf.sigma_rs_ang = params["sigma_rs_ang"];
    
    if(params.contains("alpha_a")) _conf.alpha_a = params["alpha_a"];
    if(params.contains("alpha_v")) _conf.alpha_v = params["alpha_v"];
 
    if( params.contains("cam_offset_x")) _conf.cam_offset_x = params["cam_offset_x"];
    if( params.contains("cam_offset_y")) _conf.cam_offset_y = params["cam_offset_y"];

    if( params.contains("rs_global_rotation")) _conf.rs_global_rotation = params["rs_global_rotation"];

    if( params.contains("filter_window_imu")) _conf.filter_window_imu = params["filter_window_imu"];
    if( params.contains("filter_window_enc")) _conf.filter_window_enc = params["filter_window_enc"];

    if (params.contains("min_imu_accel_for_correction")) _conf.min_imu_accel_for_correction = params["min_imu_accel_for_correction"];
    if (params.contains("enable_slip_check")) _conf.enable_slip_check = params["enable_slip_check"];

    if(params.contains("aruco_is_walker_center")) _conf.aruco_is_walker_center = params["aruco_is_walker_center"];

    if(params.contains("calibration_active")) _conf.calibration_active = params["calibration_active"];
    if(params.contains("gyro_scaling")) _conf.gyro_scaling = params["gyro_scaling"];

    if(params.contains("htc")) _conf.htc = params["htc"];
